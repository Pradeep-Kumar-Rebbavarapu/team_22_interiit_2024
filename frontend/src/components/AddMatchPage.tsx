...
export default function TeamSelectionForm() {
  ...
  const fetchPlayers = useCallback(async (page: number) => {
    setIsLoading(true);
    try {
      const response = await fetch(`http://localhost:8000/backend/api/v1/players/?limit=${PLAYERS_PER_PAGE}&offset=${(page - 1) * PLAYERS_PER_PAGE}`);
      const data: PlayersResponse = await response.json();
      setAllPlayers(prevPlayers => {
        const newPlayers = data.results.filter(newPlayer => 
          !prevPlayers.some(existingPlayer => existingPlayer.id === newPlayer.id)
        );
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const requiredColumns = ['Player Name', 'Squad', 'Match Date', 'Format'];
        ...
        if (missingColumns.length > 0) {
          toast({
            variant: "destructive",
            title: "Error",
            description: `Missing compulsory columns: ${missingColumns.join(', ')}. Please ensure all required columns are present.`,
          })
          return;
        }
        ...
        toast({
          title: "Success",
          description: "Excel file processed successfully. Teams have been populated.",
        })
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await axios.post('http://localhost:8000/backend/api/v1/add-match/', submissionData);
      toast({
        title: "Success",
      ...
      <div className="container mx-auto">
        ...
        <div className="mb-6 flex items-center">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p>The Excel file must contain the following columns: Player Name, Squad, Match Date, Format.</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
        ...
        <div className="flex space-x-4">
          <div className="w-1/2 bg-white p-4 rounded-lg shadow-lg">
            <h2 className="text-xl font-semibold mb-2">Team A Players ({teamAPlayers.length}/11)</h2>
            <div className="h-[calc(100vh-350px)] overflow-y-auto">
              {(isExcelMode ? teamAPlayers : displayedPlayers).map((player, index) => (
                  ...
                  <CardContent className="flex items-center justify-between p-4">
                    ...
                    {!isExcelMode && (
                      <Checkbox
                        ...
                        onCheckedChange={() => togglePlayerSelection(player, 'A')}
                      />
                    )}
                  </CardContent>
                </Card>
          ...
          <div className="w-1/2 bg-white p-4 rounded-lg shadow-lg">
            <h2 className="text-xl font-semibold mb-2">Team B Players ({teamBPlayers.length}/11)</h2>
            <div className="h-[calc(100vh-350px)] overflow-y-auto">
              {(isExcelMode ? teamBPlayers : displayedPlayers).map((player, index) => (
                  ...
                  <CardContent className="flex items-center justify-between p-4">
                    ...
                    {!isExcelMode && (
                      <Checkbox
                        ...
                        onCheckedChange={() => togglePlayerSelection(player, 'B')}
                      />
                    )}
                  </CardContent>
                </Card>
        ...
        <div className="mt-4 text-center">
          <Button 
            ...
            disabled={isLoading || teamAPlayers.length !== 11 || teamBPlayers.length !== 11 || !teamAName || !teamBName}
